...
from parser.utils.rds_default_config import RDSDefaultConfig  # Use defaults
...
class RDSStack(Stack):
    def __init__(self, scope: Construct, resources, config: dict, **kwargs):
        ...
        # Merge the defaults with the provided configuration
        merged_config = {key: config.get(key, RDSDefaultConfig.get(key)) for key in RDSDefaultConfig.DEFAULTS}

        # Retrieve the default VPC from resources
        vpc = resources[merged_config["vpc"]].vpc
        ...
        self.subnet_group = rds.SubnetGroup(
            ...
            vpc=vpc,
            removal_policy=removal_policy_map.get(merged_config["removal_policy"].upper(), RemovalPolicy.DESTROY),
        ...
        self.db_instance = rds.DatabaseInstance(
            ...
            instance_type=ec2.InstanceType(merged_config["instance_type"]),
            allocated_storage=merged_config["allocated_storage"],
            vpc=vpc,
            security_groups=[resources[config["security_group"]].sg],
            database_name=config.get("database_name"),
            backup_retention=Duration.days(merged_config["backup_retention_days"]),
            deletion_protection=merged_config["deletion_protection"],
            multi_az=merged_config["multi_az"],
            publicly_accessible=merged_config["publicly_accessible"],
            storage_encrypted=merged_config["storage_encrypted"],
            ...
            removal_policy=removal_policy_map.get(merged_config["removal_policy"].upper(), RemovalPolicy.DESTROY),
            cloudwatch_logs_exports=merged_config["cloudwatch_logs_exports"],
        )
